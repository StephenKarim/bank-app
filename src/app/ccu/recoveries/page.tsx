      ...
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <header className="bg-white shadow-sm">
          <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between animate-fadeInNoMove">
            <h1 className="text-xl font-bold text-gray-800">Recoveries</h1>
          ...
          <div className="grid gap-4">
            {subcategories.map((sub) => (
              <Link
                key={sub.name}
                href={sub.link}
                className="block px-6 py-3 rounded bg-red-600 text-white text-lg font-semibold hover:bg-red-700 transition"
              >
                {sub.name}
              </Link>
            ))}
